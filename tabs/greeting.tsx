    ...
    return (
        <CommonShortcutProvider>
            <div className={"w-full h-lvh"}>
                <div
                    className="w-full h-full flex items-center min-w-[1360px] overflow-x-auto px-[7.8%] justify-between"
                    style={{
                        background: `url(${GuideBg}) no-repeat center/cover`
                    }}>
                    ...
                    {guideList.map((item, index) => {
                        return (
                            <div
                                key={index}
                                className={`flex-none w-[45%] min-w-[600px] h-fit bg-white rounded-3xl p-2 flex flex-col ${step === index ? "block" : "hidden"}`}>
                                <video
                                    src={item.video}
                                    ...
                                    muted={true}
                                    loop={true}></video>
                                <div className="relative w-full flex-1   py-[3%] box-border flex flex-col items-center">
                                    <div className="min-h-[180px] h-fit w-full flex justify-center items-center">
                                        <img
                                            src={item.textImg}
                                            className="h-auto"
                                            style={{
                                                width: `${item.textImgWidth}`
                                            }}
                                            alt=""
                                        />
                                    </div>
                                    <div
                                        className="w-[29.6%] h-auto relative cursor-pointer group"
                                        onClick={handleClick}>
                                        <img
                                            src={item.btnImg}
                                    ...
                                    <div className="absolute top-[-30px] left-[50%] translate-x-[-30px] w-[60px] flex">
                                        {guideList.map((ele, i) => {
                                            return (
                                                <div
                                                    key={i}
                                                    className={`w-[10px] h-[10px] mr-1 rounded-full ${i === step ? "bg-[#0A4DFE]" : "bg-[#333333] bg-opacity-50"}`}></div>
                                            );
                                        })}
                                    </div>
                                </div>
