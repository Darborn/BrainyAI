...
export default function Base() {
    ...
    const [closeHostNames, setCloseHostNames] = useStorage<string[]>(
        "CloseHostNamesData",
        []
    );
    ...
    function checkSelection() {
        ...
        if (!selection?.isCollapsed && selectionText && selectionText.trim()) {
            ...
            void chrome.runtime.sendMessage({
                action: MESSAGE_ACTION_SET_QUOTING_SELECTION_TEXT,
                data: selectionText
            });
    ...
    async function showToolByConfig() {
        ...
        Logger.log(
            "!closeHostNames.includes(hostname)======",
            !hostNames?.includes(hostname)
        );
        if (
            !hostNames?.includes(hostname) &&
            !tempHostNames.includes(hostname)
        ) {
            setShowTool(true);
        }
    }
    ...
    const setPanelOpenOrNot = function () {
        void chrome.runtime.sendMessage({
            action: MESSAGE_ACTION_SET_PANEL_OPEN_OR_NOT
        });
    };

    const sendMessageQuotingCancel = function () {
        void chrome.runtime.sendMessage({
            action: MESSAGE_ACTION_SET_QUOTING_CANCEL
        });
    };
    ...
    const askBarContentCopy = function (
        e: React.MouseEvent<HTMLImageElement, MouseEvent>
    ) {
        e.stopPropagation();
        Logger.log("askAi=========");
    ...
    const quickBarHeaderClick = function (
        e: React.MouseEvent<HTMLImageElement, MouseEvent>
    ) {
        e.stopPropagation();
        Logger.log("askAi=========");
    ...
    useEffect(() => {
        ...
        if (!openInPlugin(location.href)) {
            ...
            document.body.addEventListener("mousedown", () => {
                ...
                if (!popIsShowByShortcuts) {
                    Logger.log(
                        `mousedown popIsShowByShortcuts=============${popIsShowByShortcuts}`
                    );
                    setShowTool(false);
                    sendMessageQuotingCancel();
                }
            ...
            document.body.addEventListener("keydown", (e) => {
                if (e.shiftKey && e.metaKey && e.key === "Enter") {
                    Logger.log(
                        "viewGroup shiftKey and metaKey and Enter =============="
                    );
                    goToSearchByAskBar()
                        .then(() => {
    ...
    const popupPrompt = (
        <div className={baseContentStyle.popupPrompt}>
            <div className={baseContentStyle.header}>
                ...
                <img
                    className={baseContentStyle.iconImage}
                    src={popupSettingIcon}
                    alt=""
                    onClick={() => {
                        window.open(
            ...
            <List
                ...
                renderItem={(car, index) => {
                    ...
                    return (
                        <List.Item
                            className={baseContentStyle.listItem}
                            ...
                            <div className={baseContentStyle.listContent}>
                                <div className={baseContentStyle.leading}>
                                    ...
                                    <div
                                        className={
                                            baseContentStyle.leadingText
                                        }>
                                        {car.title}
                                    </div>
                                </div>
                                <img
                                    className={baseContentStyle.pinIcon}
                                    src={
                                        car.isSelect
                                            ? DingSelectIcon
                                            : DingUnSelectIcon
    ...
    const dealQuickBarVisibleConfig = function (
    ...
    const popupQuickPromptConfig = (
        <div className={baseContentStyle.popupQuickConfig}>
            <div onClick={(e) => dealQuickBarVisibleConfig(e, 1)}>
                Hide until Next visit
            </div>
            <div onClick={(e) => dealQuickBarVisibleConfig(e, 2)}>
        ...
        <div>
            {
                <div
                    ...
                    <div
                        ...
                        className={
                            "bg-white shadow-[0_4px_12px_0px_rgba(0,0,0,.2)] z-[1] overflow-hidden rounded-[8px] h-[32px] py-[4px] items-center"
                        }>
                        <div
                            className={
                                "pl-[4px] box-border flex justify-center cursor-pointer items-center"
                            }>
                            <div
                                className={
                                ...
                                <img
                                    onClick={quickBarHeaderClick}
                                    className={
                                        "block w-[20px] h-[20px] mr-[4px]"
                                    }
                                    src={PupHeaderIcon}
                                    alt=""
                                />
                            ...
                            <div
                                className={
                                    "w-[1px] h-[24px] bg-[#000000] opacity-20 mr-[4px]"
                                }></div>
                        </div>
                        <div>
                            <SearchBar
                                ...
                                isVisible={visiblePop ?? false}
                                onOpenChange={(visiblePopup) => {
                                    if (visiblePopup) {
                                        selectPopType = 1;
                                    }
                                    popIsShowByShortcuts = visiblePopup;
                                    Logger.log(
                                        `popIsShowByShortcuts=================${popIsShowByShortcuts}`
                                    );
                                    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
                                    // @ts-expect-error
                                ...
                                onItemClick={(id) => {
                                    goToAskEngine(selectedText, id, undefined);
                                }}
                                onItemSearchClick={() => {
                                    goToSearch(selectedText);
                                }}
                            />
                        </div>
                        <div
                            className={
                                "w-[1px] h-[24px] bg-[#000000] opacity-20 ms-[8px]"
                            }></div>
                        ...
                        <div
                            ...
                            <img
                                className={
                                    "w-[20px] h-[20px] ms-[8px] me-[8px] cursor-pointer"
                                }
                                src={SmallAskAiIcon}
                                onMouseEnter={() => {
                                    setVisibleAsk(true);
                            ...
                            {visibleAsk && (
                                <div
                                    className={
                                        "text-[#0A4DFE] text-[12px] font-[400] justify-start items-center me-[16px]"
                                    }>
                                    ⌘ + J
                                </div>
                            )}
                    ...
                    <Popover
                        ...
                        title={null}
                        content={popupQuickPromptConfig}
                        arrow={false}
                        placement="bottomLeft"
                        open={quickConfigOpen}
                        onOpenChange={(isOpen) => {
                            Logger.log(
                                `quickConfigOpen=================${isOpen}`
                            );
                        ...
                        {visibleAsk && (
                            <img
                                className={
                                    "w-[16px] h-[16px] absolute top-0 right-0 cursor-pointer"
                                }
                                src={askCloseIcon}
                                alt=""
                                onClick={closeTool}
            ...
            <div
                onClick={setPanelOpenOrNot}
                className={
                    "fixed group right-[-50px] hover:quick-text:block transition-all bg-white hover:bg-[#CEDBFF] hover:right-0 cursor-pointer bottom-[18%] flex items-center rounded-l-[20px] h-[40px] w-[90px] shadow-[0_4px_24px_0px_rgba(0,0,0,.2)]"
                }>
                <img
                    className={"block  w-[24px] h-[24px] ml-[8px]"}
                    src={Icon}
                    alt=""
                />
                <div
                    className={
                        "quick-text text-[15px] hidden group-hover:block ml-[8px] text-[#0A4DFE]"
                    }>
                    ⌘ + I
                </div>
            </div>
            ...
            <div
                onClick={() => showAskBar()}
                className={
                    "fixed right-[6px] transition-all hover:right-[8px] cursor-pointer bottom-[calc(18%+65px)] flex justify-center items-center bg-white rounded-full h-[32px] w-[32px] shadow-xl transform hover:scale-110"
                }>
                <CTooltip
                    title={"⌘ + J"}
                    ...
                    <img
                        className={"block w-[20px] h-[20px] mx-auto my-auto"}
            ...
            {showAskSearch && (
                <div
                    ...
                    className={
                        "bg-white shadow-[0_4px_12px_0px_rgba(0,0,0,.2)] overflow-hidden rounded-[8px] relative flex flex-col"
                    }>
                    <div
                        style={{ display: showAskContent ? "flex" : "none" }}
                        className={
                            "justify-start bg-[#F2F5FF] leading-[24px] p-[16px] text-black font-weight-400 text-[15px]"
                        }>
                        <p>
                            {selectedText}
                            <CTooltip
                    ...
                    <div
                        ...
                        <Input.TextArea
                            ...
                            className={
                                "p-[16px] border-none font-sans text-[15px] text-black leading-tight focus:outline-none focus:shadow-outline bg-transparent align-top overflow-auto whitespace-pre-wrap resize-none"
                            }
                        ...
                        <div
                            className={
                                "flex flex-row justify-between mt-[8px] me-[16px] items-center mb-[8px]"
                            }>
                            <div
                                className={
                                    "h-[25px] text-[#C2C2C2] bg-[#F3F4F9] rounded-tr-[8px] rounded-br-[8px] px-[8px] py-[4px] text-[12px] font-[400] me-[12px] whitespace-nowrap cursor-pointer flex justify-center items-center"
                                }
                                onClick={() => sendAskAIDefault()}>
                                {"⏎ AskAI"}
                            </div>
                            <SearchBar
                                cards={cards}
