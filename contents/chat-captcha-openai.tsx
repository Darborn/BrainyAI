...
export default function OpenAiChatInStandaloneChallengeWindow() {
    ...
    const isOpenInPlugin = new URLSearchParams(location.search).get(
        IS_OPEN_IN_PLUGIN
    );
    ...
    const checkCaptchaSuccess = function () {
        ...
        if (canonicalElement.length) {
            setCaptchaSucceed(
                canonicalElement[0]?.textContent?.toLowerCase() === "chatgpt"
            );
        }
    };
    ...
    return (
        <div>
            {isOpenInPlugin && !captchaSucceed && (
                <ChatCaptchaBanner
                    text={""}
                    windowKey={OpenaiBot.CAPTCHA_WINDOW_KEY}
                />
            )}
            {captchaSucceed && (
                <ChatCaptchaBanner
