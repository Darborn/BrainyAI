...
export const SidePanelContext = React.createContext<ISidePanelContext>(
    {} as ISidePanelContext
);
...
const Sites = [
    ...
    new ISearchSiteItem(
        IonMetaso,
        SiteName.METASO,
        `https://metaso.cn/search/{session_id}`,
        "https://metaso.cn/"
    ),
    new ISearchSiteItem(
        IconYou,
        SiteName.YOU,
        "https://you.com/search?q={query}&fromSearchBar=true&tbm=youchat&chatMode=default",
        "https://you.com/"
    ),
    new ISearchSiteItem(
        IconDevv,
        SiteName.DEVV_,
        `https://devv.ai/?${DEVV_SEARCH_KEY}={query}`,
        "https://devv.ai/"
    ),
    new ISearchSiteItem(
        IconPhind,
        SiteName.PHIND,
        "https://www.phind.com/search?q={query}&ignoreSearchResults=false",
        "https://www.phind.com/search?home=true"
    ),
    new ISearchSiteItem(
        IconExa,
        SiteName.EXA,
        "https://exa.ai/search?c=all&q={query}",
        "https://exa.ai/"
    )
...
const SidePanelProvider = ({ children }: { children: ReactNode }) => {
    const [searchSites] = useState<ISidePanelContext["searchSites"]>(Sites);
    const [windowHeight, setWindowHeight] =
        useState<ISidePanelContext["windowHeight"]>(0);
    const [currentSearchSiteItem, setCurrentSearchSiteItem] = useState<
        ISidePanelContext["currentSearchSite"]
    >(searchSites[0]);
    const [searchText, setSearchText] =
        useState<ISidePanelContext["searchText"]>("");
    // const setSearchTextByEncode = (text: string) => {
    //     Logger.log(`setSearchText==============${text}`)
    //     setSearchText(encodeURIComponent(text))}
    const [searchSitesWithKey, setSearchSitesWithKey] = useState(
