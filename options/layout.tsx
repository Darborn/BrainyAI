...
export default function Layout() {
    ...
    const ImageTextComponent = ({
        ...
        <div className={`w-full flex flex-row ${className}`} onClick={onClick}>
            <img
                src={isSelected ? imageSrcSelected : imageSrc}
                className={"w-[24px] h-[24px"}
            />
            <div
                className={`text-[15px] ${isSelected ? "text-[#0A4DFE]" : "text-[#5E5E5E]"} ml-[12px]`}>
                {text}
            </div>
        </div>
    );
    ...
    return (
        <div className={"max-w-[1206px] min-w-[1180px] px-[24px] flex m-auto"}>
            <div
                className={
                    "w-[319px] bg-white cursor-pointer flex-shrink-0 flex-grow-0"
                }>
                <div className={"w-full flex flex-row mt-[64px] ml-[24px]"}>
                    <img src={Icon} className={"w-[32px] h-[32px]"} />
                    <div
                        style={{ fontWeight: 500 }}
                        className={"text-[20px] text-[#0A4DFE] ml-[10px]"}>
                        BrainyAI
                    </div>
                </div>
