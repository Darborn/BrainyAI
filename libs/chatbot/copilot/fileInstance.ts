...
export class CopilotFileSingleton implements BotFileInstance<CopilotUploadResponse> {
    ...
    async uploadFile(
        ...
        if (initErr) {
            this.refs[this.tempRefKey] = new FileRef<CopilotUploadResponse>(
                initErr,
                null
            );
        ...
        const imageBase64 = (await this.convertImageToBase64(file)).split(
            ","
        )[1];

        const [err, uploadRes]: [ChatError, CopilotUploadResponse] =
