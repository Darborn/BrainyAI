...
import {
    KimiFileSingleton,
    KimiSupportedMimeTypes
} from "~libs/chatbot/kimi/fileInstance";
...
export class KimiBot extends BotBase implements IBot {
    ...
    static desc =
        "Suitable for online text generation, chatbots, text summarization, and creative writing.";
    botSession: KimiSessionSingleton;
    fileRefs: string[];

    constructor(params: BotConstructorParams) {
        super(params);
        this.botSession = KimiSessionSingleton.getInstance(
    ...
    private async streamChat(
        ...
        myHeaders.append(
            "referer",
            "https://kimi.moonshot.cn/chat/" + conversationId
        );
        ...
        fetch(
            `https://kimi.moonshot.cn/api/chat/${conversationId}/completion/stream?--kkm=1`,
            {
                method: "POST",
                headers: myHeaders,
                body: raw,
                            ...
                            if (raw !== "") {
                                try {
                                    ...
                                    if (event === "cmpl") {
                                        ...
                                        _this.botSession.session.addMessage(
    ...
    private async createConversation(): Promise<
        [ChatError?, KimiCreateConversation?]
    > {
