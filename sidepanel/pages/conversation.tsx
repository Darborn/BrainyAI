...
} from "~provider/sidepanel/ConversationProvider";
...
import ModelManagementProvider, {
    ModelManagementContext,
    type M,
    type Ms
} from "~provider/ModelManagementProvider";
import { ErrorCode, type ChatError } from "~utils/errors";
...
import {
    ...
    PdfPromptDatas,
    PromptDatas
} from "~options/constant/PromptDatas";
...
const md = markdownit({
    highlight: function (str, lang) {
        if (lang && hljs.getLanguage(lang)) {
            try {
                return (
                    '<pre><code  style="background-color: #F6F8FA" class="hljs">' +
                    hljs.highlight(str, {
                        language: lang,
                        ignoreIllegals: true
                    }).value +
...
const PromptTags = ({ cards, itemClick, onVisibleTagChange }) => {
    ...
    function isElementInViewportX(el: HTMLElement) {
        ...
        return (
            rect.left >= 0 &&
            rect.right <=
                (window.innerWidth || document.documentElement.clientWidth) - 30
        );
    }
    ...
    return (
        <div
            ref={rootRef}
            className={
                "flex-row-reverse flex w-full justify-start items-center overflow-hidden hideScrollBar"
            }>
            {cards?.map((item, index) => {
                const ItemIcon = getIconSrc(item.imageKey);
                Logger.log(
                    `item.imageKey:==========${item.imageKey}==========${item.imageKey != null}`
                );
                return (
                    <div
                        key={index}
                        ...
                        style={{
                            visibility: visibleTag.includes(index)
                                ? "visible"
                                : "hidden"
                        }}>
                        <div
                            className={
                                "w-[1px] h-[25px] me-[2px] tagAnchor"
                            }></div>
                        <div
                            className={
                                "bg-[#0057FF1A] rounded-[50px] flex flex-row justify-center items-center h-[25px] px-[12px] py-[4px] ms-[8px] cursor-pointer hover:bg-[#0057FF33]"
                            }
                            onClick={(e) => itemClick(item, index, e)}>
                            {item.imageKey != null && (
                                <div
                                    className={
                                        "w-[16px] h-[16px] mr-[4px] cursor-pointer"
                                    }>
                                    {item.itemType === PromptTypes.CUSTOM ? (
                                        <ItemIcon
                                            style={{
                                                fontSize: "16px",
                                                color: "#0057FF"
                                        ...
                                        <img
                                            ...
                                            src={getImageBlueSrc(item.imageKey)}
                                            alt={""}
                                        />
                                    )}
                                </div>
                            ...
                            <div
                                className={
                                    "text-[#0057FF] text-[12px] whitespace-nowrap"
                                }>
                                {item.title}
                            </div>
                        </div>
...
const AIError = memo(function ({
    model,
    error,
    start,
    setCurrentBotResponseMessage,
    activePage
}: {
    model: M;
    error: ChatError;
    start: () => void;
    setCurrentBotResponseMessage: React.Dispatch<
    ...
    const BlueBtn = ({
        ...
        return (
            <div
                onClick={handleClick}
                className="w-fit cursor-pointer mt-4 px-6 h-[31px] bg-[#0A4DFE] rounded-[8px] text-white text-[14px] font-medium flex items-center justify-center">
                {text}
            </div>
        );
    };
        ...
        switch (error?.code) {
            case ErrorCode.REQUEST_TIMEOUT_ABORT:
                errMessage = (
                    <div className="w-fit">
                        <CatAnimation />
                        <div className="mt-4 text-[12px] leading-loose">
                            If there is no response for a long time, please try
                            to refresh manually.
                            <u
                                className="cursor-pointer text-[#0A4DFE]"
            ...
            case ErrorCode.CAPTCHA:
                errMessage = (
                    <div className="w-fit">
                        ...
                        <div>
                            You need to complete the {name} Captcha Challenge to
                            continue using it. Please proceed to the challenge.
                        </div>
            ...
            case ErrorCode.CONVERSATION_LIMIT:
                errMessage = (
                    <div className="w-fit">
                        <div className="text-base font-bold mb-2">
                            Oops! 3rd-Party Error
                        </div>
                        {error?.message ? (
                            <div className="mb-3">{error?.message}</div>
                        ) : (
                            <div className="mb-3">
                                Conversation limit reached. Please try it later.
                                Or you can check your
                                <u
                                    className={"text-[#0A4DFE] cursor-pointer"}
                                    onClick={openCaptcha}>
                                    {" "}
                                    {name}{" "}
                                </u>{" "}
                        ...
                        <u
                            className={
                                "text-[14px] text-[#0A4DFE] cursor-pointer"
                            }
                            onClick={clickRefresh}>
                            Click to Refresh
                        </u>
            ...
            case ErrorCode.MODEL_INTERNAL_ERROR:
                errMessage = (
                    <div className="w-fit">
                        ...
                        <div className="mb-3">
                            {error?.message ??
                                `This model's current conversation is no longer available. Please try to reload the ${name} service manually.`}
                        </div>
                        <u
                            className={
                                "text-[14px] text-[#0A4DFE] cursor-pointer"
            ...
            case ErrorCode.UPLOAD_FILE_NOT_SUPPORTED:
                errMessage = (
                    <div className="w-fit">
                        ...
                        <div className="mb-3">
                            This model cannot support PDF/Image analysis
                            functions. You can switch models to access this
                            function.
                        </div>
                        <u
                            className={
            ...
            case ErrorCode.FILE_OTHER:
                errMessage = (
                    <div className="w-fit">
                        ...
                        <div className="mb-3">
                            The current model encountered an error while
                            analyzing the image/PDF file. You can switch models
            ...
            default:
                errMessage = (
                    <div className="w-fit">
                        <div className="text-base font-bold mb-2">
                            Oops! The Message Went Missing
                        </div>
                        <div className="leading-loose">
                            Please refresh after check:
                        </div>
                        <ul className="list-disc leading-loose pl-4">
                            <li>
                                Ensure the internet connection is stable or try
                                to switch networks.
                            </li>
                            <li>
                                Make sure your{" "}
                                ...
                                account is available for messaging.
                            </li>
                        </ul>
    ...
    return <div className="text-[#5E5E5E]">{getError()}</div>;
});
...
const AIMessageAppendix = memo(
    ({ appendix }: { appendix?: ConversationMessageAppendix }) => {
        if (appendix) {
...
const getPrompt = function (message: ConversationMessage): string {
    return message.data.prompt
        .replaceAll(PROMPT_PLACEHOLDER_TEXT, `"${message.data.promptText}"`)
        .replaceAll(PROMPT_PLACEHOLDER_LANG, `"${message.data.lang}"`);
};
...
const AITextContent = ({
    ...
    const [currentBotResponseMessage, setCurrentBotResponseMessage] = useState<
        ConversationResponse[]
    ...
    const start = () => {
        if (message.data.prompt) {
            ...
            const fileRefToken = UploadUtils.getFileRefToken(
                message,
                bot.botName
            );
            Logger.log(
                `sendMsgInfo:===botName:${bot.botName}===prompt:${prompt}=====fileRefToken:${fileRefToken}====rid:${rid}`
            );
            void new bot({ globalConversationId: conversationId }).completion({
                ...
                cb: async (_rid, data) => {
                    ...
                    if (_rid === rid) {
                        if (data.message_type !== ResponseMessageType.DONE) {
                            ...
                            setCurrentBotResponseMessage((pre) => {
                                if (pre && pre.length > 0) {
                                    pre[pre.length - 1].appendix =
                                        data?.appendix;
                                    pre[pre.length - 1].message_type =
                                        data.message_type;
                                }
                ...
                file: message.data.uploadFile?.length
                    ? (message.data.uploadFile[4] ?? undefined)
                    : undefined,
    ...
    const copyClick = async () => {
        navigator.clipboard
            .writeText(
                currentBotResponseMessage[activePage - 1].message_text ?? ""
            )
            .then(() => {
    ...
    const pageChange = (direction) => {
        ...
        if (
            direction === "r" &&
            activePage < currentBotResponseMessage.length
        ) {
            setActivePage(activePage + 1);
        }
    };

    const getCardOperateArea = function (
        isLastChatInList: boolean,
                ...
                {!item.error && (
                    <div
                        ...
                        <div className="pl-[16px] flex items-center">
                            {isLastChatInList && (
                                <CTooltip title={"Regenerate"}>
                                    <img
                                        src={RefreshIcon}
                                        className="w-[16px] h-[16px] mr-[8px] cursor-pointer"
                        ...
                        {isLastChatInList && (
                            <div className="pr-[16px] flex items-center">
                                ...
                                <div className="mx-[8px] text-[12px] text-[#5E5E5E]">
                                    ...
                                    <span> / </span>
                                    <span className="">
                                        {currentBotResponseMessage.length}
                                    </span>
                                </div>
                                <img
                                    src={
    ...
    const getMessageContent = function (
        ...
        if (item && item.message_text) {
            return (
                <ErrorBoundary>
                    <YourComponent markdownText={item.message_text} />
                </ErrorBoundary>
            );
        }
        ...
        if (
            !item.error &&
            !item.message_text &&
            item.message_type !== ResponseMessageType.DONE
        ) {
            return <Thinking />;
        }
        ...
        <div>
            {currentBotResponseMessage.map((item, index) => {
                return (
                    <div
                        className={
                            activePage === index + 1 ? "block" : "hidden"
                        }
                        key={index}>
...
export const AIMessage = memo(
    ({ message, i }: { message: ConversationMessage; i: number }) => {
        ...
        } = useContext(ModelManagementContext);
        const [errorModels, setErrorModels] = useState<errorModels[]>([]);
        const [botProviders, setBotProviders] = useState<Ms>(
            message.botProviders || []
        );
            ...
            EventBus.on("errorManagement", (errorShow: boolean, model: M) => {
                if (model) {
                    if (errorShow) {
                        setErrorModels((pre) => [
                            ...pre,
                            { modelName: model.botName }
                        ]);
                    } else {
                        setErrorModels((pre) =>
                            pre.filter(
                                (item) => item.modelName !== model.botName
                            )
                        );
                    }
                }
        ...
        const singleModelCheck = async (model: M) => {
            setErrorModels((pre) =>
                pre.filter((item) => item.modelName !== model.botName)
            );
        ...
        const popoverContent = () => {
            return (
                <div ref={popoverRef}>
                    {allModels.current
                        .filter((item) => !currentBots.includes(item))
                        .map((model, index) => {
                            return (
                                <div
                                    onClick={() => singleModelCheck(model)}
                                    key={index}
                                    className={`group relative w-[310px] h-10 px-4 box-border  flex justify-between items-center hover:bg-gradient-to-r hover:from-[#E5ECFF]`}>
                                    <div className="flex items-center justify-start">
                                        ...
                                        {model.paidModel && (
                                            <div className="h-5 box-border px-2 py-1 rounded bg-[#C2C2C2] bg-opacity-20 text-[10px] text-[#C2C2C2] font-bold">
                                                3rd-paid
                                            </div>
                                        )}
                                    </div>
                                    <div className="flex justify-end items-center">
                                        ...
                                        {model.maxTokenLimit && (
                                            <div className="ml-1 h-5 box-border px-1 py-0.5 rounded bg-[#4948DB1A] bg-opacity-10 text-[12px] text-[#4948DB] font-medium">
                                                {Math.round(
                                                    model.maxTokenLimit / 1000
                                                )}
                                                k
                                            </div>
                                        )}
        ...
        return (
            <div
                ...
                className={style.messageItemOuter + " justify-start mb-[20px]"}>
...
export const UserMessage = ({ message }: { message: ConversationMessage }) => {
    ...
    const ItemIcon = getIconSrc(
        message.data.promptImageUri ? message.data.promptImageUri : undefined
    );
    const { messageApi } = useContext(SidePanelContext);
    ...
    const quotaUploadFileClick = () => {
        eventBus.emit("quota-uploadFile", {
            uploadData: message.data.uploadFile
        });
    };
    ...
    return (
        <div
            ref={containerRef}
            className={style.messageItemOuter + " justify-end mb-[20px]"}>
            <div className={`${style.contentArea} ${style.r} group`}>
                <div className="overflow-x-auto text-white rounded-[10px] justify-start">
                    <div className="bg-[#4948DB] text-sm p-4">
                        {message.data.promptImageTitle && (
                            <div
                                className="bg-[#333] bg-opacity-50 rounded-[50px] text-[12px] flex items-center px-3 w-fit h-6 "
                                style={{
                                    marginBottom: message.data.text ? 8 : 0
                                }}>
                                {message.data.promptImageUri &&
                                    (message.data.promptType === 2 ? (
                        ...
                        {/*{getPrompt(message)}*/}
                        {message.data.text && (
                            <div className="select-text relative whitespace-pre-wrap self leading-[1.5em] text-white text-opacity-90">
                                {message.data.text}
                            </div>
                        )}
                    </div>
                    {message.data.isHaveUploadFile && (
                        <div className={"bg-[#E7E9F0]"}>
                            {message.data.uploadFile![3] == FileTypes.Image ? (
                                <div
                                    className={
                                        "flex relative p-[4px] max-w-[200px] max-h-[300px]"
                                    }>
                                    <img
                                        className={
                                            "rounded-[10px] object-cover w-full"
                                        }
                                        src={
                                            message.data.uploadFile
                                                ? message.data.uploadFile[0]
                                                : ""
                                        }
                                        alt=""
                                    />
                                    <div
                                        className={
                                            "flex absolute right-[12px] top-[12px] flex-row-reverse justify-end "
                                        }>
                                ...
                                <div
                                    className={
                                        "flex flex-row justify-between items-center h-[60px]"
                                    }>
                                    <div>
                                        <div
                                            className={
                                            ...
                                            <div
                                                className={
                                                    "w-[48px] h-[48px] flex-shrink-0 ml-[12px] rounded-[8px] border-dashed border-[1px] border-[#0A4DFE] bg-[#E6EDFF] flex justify-center items-center"
                                                }>
                                                <div
                                                    className={
                                                        "w-[24px] h-[24px] flex justify-center items-end"
                                                    }
                                                    style={{
                                                        backgroundImage: `url(${FileBgIcon})`
                                                    }}>
                                                    <div
                                                        className={
                                                            "flex text-[#0A4DFE] text-[5px] font-[700] overflow-hidden whitespace-nowrap break-all justify-center items-center mb-[3px]"
                                                        }>
                                                        {message.data
                                                            .uploadFile![1].split(
                                                                "."
                                            ...
                                            <div
                                                className={
                                                    "text-[#0A4DFE] text-[14px] ml-[16px] line-clamp-1 text-ellipsis break-all pr-[24px]"
                                                }>
                                                {message.data.uploadFile![1]}
                                            </div>
                                        </div>
                                    </div>
                                    <div
                                        className={
                                            "flex flex-row-reverse mr-[10px]"
                                        }>
                                        <CTooltip title={"Download"}>
                                            <div
                                                className={style.actionButton}
                    ...
                    {message.data.appendix && (
                        <div className="p-4 bg-[#E5E6ED]">
                            <CTooltip
                                ...
                                <div
                                    className={
                                        "cursor-pointer line-clamp-2 text-ellipsis text-[#333333] text-opacity-60 "
                                    }>
                                    {message.data.appendix}
                                </div>
                            </CTooltip>
                ...
                {!message.data.isHaveUploadFile && (
                    <div className="group-hover:visible pr-[16px] flex items-center justify-end mt-2 invisible">
                        <CTooltip title={"Copy"}>
                            <img
                                src={CopyIcon}
                                className="w-[16px] h-[16px] mr-[8px] cursor-pointer"
    ...
    const checkSelection = () => {
        ...
        if (!selection?.isCollapsed && selectionText && selectionText.trim()) {
            ...
            if (rects) {
                ...
                setToolPositions([
                    lastPosition,
                    lastCharacterRect.bottom + window.scrollY
                ]);
        ...
        <div className={style.chatList}>
            <div>
                ...
                {showTool && (
                    <img
                        ref={quotaRef}
                        onClick={handleClickQuota}
                        className="fixed w-15 h-10"
                        src={TextQuota}
                        alt=""
                        style={{
...
function ConversationContent() {
    ...
    const [quickPromptBtn, setQuickPromptBtn] = useState<[number, string]>([
        AskPromptId,
        "AskAI"
    ]);
    ...
    >([false, false, "", "", new Map(), FileTypes.OTHERS, null]);
    const { conversationId } = useContext(ConversationContext);
    const [uploadBorderColor, setUploadBorderColo] = useState("#C2C2C2");
    const [uploadBackgroundColor, setUploadBackgroundColor] =
        useState("#FFFFFF");

    useEffect(() => {
        eventBus.on("quota-click", ({ title, content }) =>
            showQuotingText(title, content)
        );
    ...
    async function refreshUploadFile(newOpen: boolean) {
        ...
        if (
            !newOpen &&
            isUploadingInfo[0] &&
            currentBotsTemp.length > 0 &&
            isUploadingInfo[6] != null &&
            !Bots.every((bot) => currentBotsTemp.includes(bot))
        ) {
    ...
    const currentModelView = () => {
        return (
            <div
                ...
                className={
                    "cursor-pointer rounded-[40px] bg-[#F3F4F9] px-3 text-[12px] h-[25px] flex justify-center items-center"
                }>
                {currentBots.length &&
                        ...
                        return (
                            <div
                                ...
                                {currentBots.length === 1 &&
                                    `${item.botName}(${getModelLoginTag(item)})`}
                            </div>
                        );
                    })}
    ...
    const handleMouseDown = (e: MouseEvent) => {
        ...
        if (popIsShow) {
            ...
            if (popComponent && !popComponent.contains(e.target)) {
                Logger.log(
                    `mousedown closePromptPopup=============${popIsShow}`
                );
                ...
                function (tabs) {
                    ...
                    if (currentTabId) {
                        void chrome.tabs.sendMessage(currentTabId, {
                            action: MESSAGE_ACTION_SET_QUOTING_SELECTION_CLEAR_CURSOR
                        });
                    }
                }
            );
    ...
    async function itemClick(
        ...
        if (isQuotClick) {
            if (isUploadingInfo[0] && isUploadingInfo[4].size > 0) {
                goToAskEngine(null, car, undefined, true, [
                    isUploadingInfo[2],
                    isUploadingInfo[3],
                    isUploadingInfo[4],
                    isUploadingInfo[5],
            ...
            if (
                inputText &&
                isUploadingInfo[0] &&
                isUploadingInfo[4].size > 0
            ) {
                goToAskEngine(inputText, car, undefined, true, [
                    isUploadingInfo[2],
                    isUploadingInfo[3],
                    isUploadingInfo[4],
                    isUploadingInfo[5],
    ...
    async function goAskAi() {
        ...
        if (inputText && inputText.trim()) {
            const card =
                quickPrompt[0] == AskPromptId
                    ? AskPromptData
                    : cards.find((card) => card.id === quickPrompt[0]);
            if (isUploadingInfo[0] && isUploadingInfo[4].size > 0) {
                goToAskEngine(inputText, card, undefined, true, [
                    isUploadingInfo[2],
                    isUploadingInfo[3],
                    isUploadingInfo[4],
                    isUploadingInfo[5],
    ...
    function goToAskEngine(
        ...
        if (card.id != AskPromptId) {
            Logger.log(
                `goToAskEngine===============${msg ? msg : "msg==null"}`
            );
    ...
    async function fileUpload(file: File) {
        const fileType = UploadUtils.isImageFile(file)
            ? FileTypes.Image
            : FileTypes.OTHERS;
    ...
    const PopupPrompt = ({ isQuotClick }) => (
        <div
            ...
            <List
                itemLayout="vertical"
                dataSource={
                    !isQuotClick && quickPromptBtn[0] != AskPromptId
                        ? [AskPromptData, ...cards]
                        : cards
                }
                bordered={false}
                ...
                style={{
                    ...
                    overflow: "auto",
                    display:
                        isHaveQuotingText && !isQuotClick ? "none" : "block"
                }}
                ...
                renderItem={(car, index) => {
                    ...
                    return (
                        <List.Item
                            style={{ height: "40px" }}
                            className={"hover:bg-[#F2F5FF] box-border"}
                            onClick={(e) =>
                                itemClick(car, index, isQuotClick, e)
                            }>
                            <div
                                style={{
                                    display: "flex",
            ...
            <div
                style={{
                    display:
                        !isUploading[0] && !isHaveQuotingText && !isQuotClick
                            ? "block"
                            : "none"
                }}
            ...
            <div
                ...
                className={
                    "h-[40px] box-border w-full flex flex-row justify-between items-center hover:bg-[#F2F5FF]"
                }
                ...
                <div
                    className={
                        "w-fit text-right text-[#C2C2C2] text-[12px] mr-[16px]"
                    }>
                    {"⇧ ⌘ ⏎"}
                </div>
            </div>
            ...
            <div className={style.mainInputArea}>
                <div className={style.chatInputTopBar}>
                    <div className={"flex flex-row justify-start items-center"}>
                        ...
                        {currentModelView()}
                        <img
                            src={InputAttachmentIcon}
                            alt=""
                            className={
                                "w-[16px] h-[16px] ms-[12px] cursor-pointer"
                            }
                ...
                <div
                    className={
                        "flex flex-col items-center relative bg-white min-h-[97px] w-full rounded-[16px] border-[1px] pb-[4px] border-solid border-[#0A4DFE] cursor-text"
                    }
                    style={{
                        borderColor: inputWarnShow ? "#FE0A36" : "#0A4DFE"
                    }}>
                    {isHaveQuotingText && (
                        <div className={"flex flex-col items-center w-full"}>
                            <div
                                className={
                                    "bg-[#F3F4F9] rounded-[16] flex w-[calc(100%-8px)] flex-row justify-between my-[4px] mx-[4px] box-border relative"
                                }>
                                <div
                                    className={
                                        "flex-col flex justify-start items-start break-words pr-[11px] overflow-hidden"
                                    }>
                                    <div
                                        className={
                                            "ms-[11px] mt-[5px] text-[#5E5E5E] text-[12px] font-[600] line-clamp-1 text-ellipsis w-full"
                                        }>
                                        {quotingText[0]}
                                    </div>
                                    <div
                                        className={
                                            "ps-[11px] mt-[1px] text-[#5E5E5E] text-[12px] font-[400] line-clamp-2 text-ellipsis mb-[4px] box-border w-full"
                                        }>
                                        {quotingText[1]}
                                    </div>
                                </div>
                                <img
                                    className={
                                        "w-[16px] h-[16px] cursor-pointer absolute top-[5px] right-[8px]"
                                    }
                                    src={ChatInputCloseIcon}
                                    alt=""
                                    onClick={(e) => {
                                        e.stopPropagation();
                                        hideQuotingText();
                            ...
                            <div
                                className={
                                    "flex-row flex w-full justify-between items-center mt-[8px] overflow-hidden"
                                }>
                                <PromptTags
                                    cards={cards}
                                    ...
                                    itemClick={(
                                        ...
                                        itemClick(car, index, true, e)
                                    }></PromptTags>
                                <Popover
                                    title={null}
                                    align={{ offset: [0, 20] }}
                                    content={<PopupPrompt isQuotClick={true} />}
                                    arrow={false}
                                    placement="topLeft"
                                    overlayInnerStyle={{
                                        paddingLeft: 0,
                                    ...
                                    <img
                                        className={
                                            "w-[20px] h-[20px] ms-[4px] me-[4px] cursor-pointer"
                                        }
                                        src={TriangleIcon}
                                        alt=""
                                        style={{
                                            visibility:
                                                Math.max(...promptVisibleTag) +
                                                    1 <
                                                cards.length
                                                    ? "visible"
                                                    : "hidden"
                            ...
                            <div
                                className={
                                    "w-[calc(100%-16px)] h-[1px] bg-[#F6F6F6] mt-[8px]"
                                }
                            />
                        </div>
                    )}
                    {isUploading[0] && (
                        <div className={"flex flex-col w-full"}>
                            <div
                                className={
                                    "flex flex-row justify-between w-auto m-[8px] h-[56px] rounded-[12px] border-[1px] border-[#F3F4F9] items-center"
                                }>
                                <div
                                    className={
                                        "flex flex-row justify-start items-center overflow-hidden"
                                    ...
                                    <div
                                        className={
                                            "w-[48px] h-[48px] my-[4px] ml-[4px]"
                                        }>
                                        {isUploading[1] ? (
                                            <div
                                                className={
                                                    "w-full h-full border-[1px] border-[#C2C2C2] border-dashed flex justify-center items-center rounded-[8px]"
                                                }>
                                                <LoadingOutlined
                                                    className={
                                                        "text-[32px] text-[#0A4DFE]"
                                                    }
                                                />
                                            </div>
                                        ) : isUploading[5] ==
                                          FileTypes.Image ? (
                                            ...
                                            <div
                                                className={
                                                    "w-full h-full rounded-[8px] border-dashed border-[1px] border-[#0A4DFE] bg-[#E6EDFF] flex justify-center items-center"
                                                }>
                                                <div
                                                    className={
                                                        "w-[24px] h-[24px] flex justify-center items-end"
                                    ...
                                    <div
                                        className={
                                            "text-[#5E5E5E] text-[14px] ml-[16px] line-clamp-1 text-ellipsis max-w-[calc(100%-68px)] break-all"
                                        }>
                                        {isUploading[3]}
                                    </div>
                                </div>
                                <img
                                    className={
                                        "w-[16px] h-[16px] mx-[20px] cursor-pointer"
                                    }
                    ...
                    <Input.TextArea
                        ...
                        className={
                            "w-full px-[12px] py-[8px] text-black align-top bg overflow-auto whitespace-pre-wrap resize-none focus:bg-transparent focus:shadow-none focus:border-none focus:outline-none hover:bg-transparent hover:shadow-none hover:border-none hover:outline-none"
                        }
                        autoFocus={true}
                    ...
                    <div
                        className={
                            "w-full h-[23px] flex flex-row-reverse justify-start items-center mb-[8px]"
                        }>
                        <Popover
                            trigger={"click"}
                            title={null}
                            ...
                            content={<PopupPrompt isQuotClick={false} />}
                            arrow={false}
                            placement="topLeft"
                            open={isPromptPopShow}
                            onOpenChange={(visibleAskPop) => {
                                Logger.log(
                                    `visibleAskPop=============${visibleAskPop}`
                                );
                            ...
                            <img
                                className={
                                    "w-[20px] h-[20px] mr-[12px] cursor-pointer"
                                }
                                src={TriangleIcon}
                                alt=""
                            />
                        ...
                        <div
                            className={
                                "flex justify-center items-center h-[25px] text-[#C2C2C2] bg-[#F3F4F9] rounded-[8px] px-[8px] py-[4px] text-[12px] font-[400] mr-[8px] whitespace-nowrap cursor-pointer"
                            }
                            onClick={() =>
                                goAskAi()
                            }>{`⏎ ${quickPromptBtn[1]}`}</div>
                    </div>
                </div>
            </div>
            <Modal
                open={isUploadAttachment}
                className={
                    "absolute top-[calc(50%-200px)] left-0 right-0 p-0 modelUpload bg-transparent overflow-hidden"
                }
                footer={[]}
                closable={false}
                width="600px">
                <div
                    className={
                        "flex flex-col justify-start w-full h-full bg-[#F3F4F9] rounded-[16px] overflow-hidden"
                    }>
                    <div
                        className={
                            "flex flex-row justify-between items-center px-[16px] mt-[12px]"
                        }>
                        <div
                            className={"text-[#C2C2C2] text-[16px] font-[700]"}>
                            Upload file
                        </div>
                        <img
                            className={"w-[16px] h-[16px] cursor-pointer"}
                    ...
                    <div
                        className={
                            "flex flex-col w-auto justify-start bg-white rounded-[16px] ml-[4px] mr-[4px] mb-[4px] mt-[12px]"
                        }>
                        <div
                            className={
                                "text-[#5E5E5E] text-[14px] mt-[16px] mx-[24px]"
                            }>
                            Upload a file to receive intelligent answers to your
                        ...
                        <div
                            ...
                            <Dragger
                                ...
                                <p className="ant-upload-text text-[#333333]">
                                    Drag and drop here to upload or
                                </p>
                                <p className="text-[16px] mb-[4px] text-[#0A4DFE] underline block">
                                    click to upload
                                </p>
                                <p className="ant-upload-hint">
                                    Depending on the different models, support
                                    for file formats such as PDF, TXT, PNG, etc.
                                    (max size 20MB)
                                </p>
                            </Dragger>
                        </div>
