...
export default function ({ naviItem }: { naviItem: IDrawerNaviItem }) {
    ...
    const coloredText = function () {
        if (isActive) {
            return (
                <div className={"text-[12px] text-[#0A4DFE]"}>
                    {naviItem.name}
                </div>
            );
        }
        ...
        return (
            <div className={"text-[12px] text-[#5E5E5E]"}>{naviItem.name}</div>
        );
    };
    ...
    return (
        <div
            ...
            <img
                className={"w-[16px] h-[16px] block mb-[6px]"}
                src={isActive ? naviItem.activeIcon : naviItem.icon}
            />
            {coloredText()}
        </div>
    );
