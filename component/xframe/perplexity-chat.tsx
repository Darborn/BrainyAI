...
function FrameContent({ src, goBack }: { src: string; goBack(): void }) {
    ...
    return (
        <div className={"absolute bg-white z-[9999]"} style={{ width, height }}>
            {(loadError || !frameLoaded) && (
                <div
                    style={{
                        width: `100%`,
                        background:
                    ...
                    className={
                        "py-[10px] flex justify-center items-center text-[14px] px-[16px] box-border absolute z-10"
                    }>
                    <div
                        onClick={goBack}
                        className={
            ...
            {!frameLoaded && (
                <div
                    className="w-20 h-20 absolute z-0 left-0 top-0 right-0 bottom-0 m-auto"
                    ref={catRef}></div>
            )}
            <iframe
                onLoad={(e) => {
...
export default class XFramePerplexityChat {
    ...
    render() {
        ...
        ReactDOM.createRoot(div).render(
            <FrameContent goBack={this.goBack.bind(this)} src={this.src} />
        );
    }
