...
const CopilotImageCreateAppendixView = memo(function ({
    src
}: {
    src: string;
}) {
    ...
    return (
        <div className={"w-full overflow-x-auto relative"}>
            {!frameLoaded && (
                <div className={"absolute left-[10px] top-[10px] text-[18px]"}>
...
export class CopilotImageCreateAppendix extends ConversationMessageAppendix {
    ...
    constructor(text: string, messageId: string) {
        ...
        this.imageCreateSingleton = CopilotImageCreateSingleton.getInstance(
            text,
            messageId
        );
    }
    ...
    render() {
        return (
            <CopilotImageCreateAppendixView
                src={this.imageCreateSingleton.createFrameUrl(
                    this.text,
                    this.messageId
                )}
            />
