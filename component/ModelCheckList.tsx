...
const ModelItems = ({ item }: { item: CMsItem }) => {
    ...
    useEffect(() => {
        ...
        window.addEventListener("mousedown", (e: MouseEvent) => {
            if (
                labelTipRef.current &&
                !labelTipRef.current.contains(e.target as Node)
            ) {
    ...
    const labelTip = (item: CMsItem) => {
        return (
            <div
                ref={labelTipRef}
                className="px-1 py-[6px] text-[14px] text-white text-opacity-60 flex justify-start items-center">
                <LockAnimation />
                <div className="ml-1">Log into&nbsp;</div>
                <u
                    className="text-white cursor-pointer"
                    onClick={() => {
                        openLogin(item.models[0]);
                    }}>
                    {new URL(item.models[0].loginUrl).hostname}
                </u>
                &nbsp;to use
            </div>
        );
        ...
        <div className="text-[#333333] mb-3">
            <div className="px-4 py-2 flex justify-start items-center">
                ...
                <Tooltip
                    ...
                    placement={"topLeft"}
                    open={isOpenProviderToolTip}>
                    <span className="text-[14px] text-[#C2C2C2] leading-none">
                        {item.label}
                    </span>
                </Tooltip>
            </div>
            ...
            {item.models
                .slice(
                    0,
                    isShowMore && item.models.length > 3
                        ? item.models.length
                        : 3
                )
                .map((model) => {
                    return (
                        <ModelItem
                            ...
                            model={model}
                            isLogin={isLogin}
                            setIsOpenProviderToolTip={setIsOpenProviderToolTip}
                            getLoginStatus={getLoginStatus}></ModelItem>
                    );
                })}
            {item.models.length > 3 && !isShowMore && (
                <div
                    className="w-full flex justify-center items-center my-2 cursor-pointer"
                    onClick={() => showMore(true)}>
                    <img src={ShowMore} className="w-4 h-4 mr-1" alt="" />
                    <span className="text-[12px] text-[#C2C2C2] mr-1">
                        {item.models.length - 3} models remaining
                    </span>
...
const ModelItem = ({
    model,
    isLogin,
    setIsOpenProviderToolTip,
    getLoginStatus
}: {
    model: M;
    isLogin: boolean;
    setIsOpenProviderToolTip: React.Dispatch<React.SetStateAction<boolean>>;
    getLoginStatus: () => void;
}) => {
    const [modelCanUse, setModelCanUse] = useState(false);
    const { conversationId } = useContext(ConversationContext);
    const { currentBots, setCurrentBots, saveCurrentBotsKeyLocal } = useContext(
        ModelManagementContext
    );
    ...
    const modelChange: (e, model) => void = async (e, model) => {
        if (!e.target.checked && currentBots.length > 1) {
            setCurrentBots(
                currentBots.filter((item) => item.botName !== model.botName)
            );
        }
    ...
    const modelTip = (model: M) => {
        return (
            <div className="px-1 py-[6px] text-[14px]">
                ...
                <div className="mb-2 text-white text-opacity-60">
                    ...
                    <u
                        ...
                        {new URL(model.loginUrl).hostname}
                    </u>
                    {modelCanUse
                        ? ""
                        : " and confirm you can access this model"}
                </div>
    ...
    return (
        <div
            className={`relative w-full h-9 px-4 box-border  hover:bg-[#F2F5FF] flex justify-between items-center`}>
            <div className="flex items-center justify-start">
                    ...
                    <div className="flex items-center">
                        <div className="w-4 h-4 mr-2 relative">
                            ...
                            {!modelCanUse && (
                                <img
                                    src={IconLock}
                                    className="w-4 h-4 absolute right-[-8px] bottom-[-6px]"
                                    alt=""
                                />
                            )}
                        </div>
                ...
                {model.paidModel && (
                    <Tooltip
                        title={
                            "This model needs to be paid at the provider’s website."
                        }>
            ...
            <div className="flex justify-end items-center">
                {model.supportUploadPDF && (
                    <Tooltip title={"This model can analyze PDF document."}>
                        <img className="ml-1 w-5 h-5" src={IconPdf} alt="" />
                    </Tooltip>
                )}
                {model.supportUploadImage && (
                    <Tooltip title={"This model can analyze image."}>
                        <img className="ml-1 w-5 h-5" src={IconPic} alt="" />
                    </Tooltip>
                )}
                {model.maxTokenLimit && (
                    <Tooltip
                        title={
                            "This model supports up to 128.000 tokens in a single session"
                        }>
                        <div className="flex justify-center items-center ml-1 h-5 rounded bg-[#4948DB1A] bg-opacity-10 text-[12px] text-[#4948DB] font-medium px-1 leading-none">
                            {formatTokenLimit(model.maxTokenLimit)}k
                        </div>
                    </Tooltip>
                )}
            </div>
    ...
    return (
        <div>
            <div className="flex justify-between items-center h-[64px] sticky top-0 z-10 px-4 bg-white">
                <span className="text-[20px] text-[#333333] font-bold">
                    AI Models
                </span>
                <CloseCircleOutlined
                    onClick={onClose}
                    style={{ fontSize: "24px", color: "#5E5E5E" }}
                />
            </div>
                        ...
                        return (
                            <div
                                ...
                                <img
                                    ...
                                    alt=""
                                />
                                <div className="text-[12px] text-[#333333] mr-3 truncate">
                                    {item.botName}
                                </div>
            ...
            {categoryModels.current.map((item, i) => {
                return <ModelItems key={i} item={item} />;
            })}
        </div>
    );
};
