...
const runConversation = async function (): Promise<[ChatError | null, any]> {
    ...
    // })
    const r = await customChatFetch(
        "https://copilot.microsoft.com/turing/userconsent?bundleVersion=1.1678.0&isStartOfConversation=true",
        {
            method: "GET",
            headers: myHeaders,
            redirect: "follow"
...
const createConversation = async function (): Promise<
    [ChatError | null, CopilotConversation | null]
> {
    const myHeaders = new Headers();
    ...
    try {
        ...
        const conversationSignature =
            headers?.get("X-Sydney-Conversationsignature") || "";
        const encryptedConversationSignature = headers?.get(
...
const handler: PlasmoMessaging.MessageHandler = async (req, res) => {
    ...
    try {
        if (
            conversationId &&
            lastConversation?.conversationId === conversationId
        ) {
        ...
        const [conversationCreateErr, conversation] =
            await createConversation();

        Logger.trace(
            "init-copilot-conversation 1",
